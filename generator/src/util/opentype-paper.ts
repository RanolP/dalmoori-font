...
function chooseProperSize(
  a: opentype.BoundingBox,
  b: opentype.BoundingBox,
): [width: number, height: number] {
...
export function makePathsForComparison(
  a: opentype.Glyph,
  b: opentype.Glyph,
): [paper.Path, paper.Path, paper.Project, [width: number, height: number]] {
...
export function toPaperPath(
  input: opentype.Glyph,
  project: paper.Project = createProject(...getSize(input.getBoundingBox())),
): paper.Path {
...
export async function renderSvg(
  path: paper.Item,
  { keepColor, svgSize, svgParams }: RenderSvgOption,
): Promise<string> {
